...
How the two BioCap devices find each other on the local network. This is the discovery layer
...
DNS-SD (DNS Service Discovery) sits on top of mDNS and organises services by type, e.g. `_http._tcp` or `_biocap._tcp` (BioCap's own type). This is the same protocol used by AirPlay, Chromecast, and network printers.
...
### `PeerMdnsService` — [data/link/PeerMdnsService.kt](../app/src/main/java/com/biocap/app/data/link/PeerMdnsService.kt)
...
```kotlin
const val SERVICE_TYPE_REGISTER = "_biocap._tcp"   // registerService()
const val SERVICE_TYPE_DISCOVER = "_biocap._tcp."  // discoverServices()
```
...
set, so duplicate calls are harmless. `stopDiscovery()` always clears `discoveredDevices` — even when
discovery was already stopped — so a stale peer never lingers after a link ends.

**MulticastLock:** one non-reference-counted lock (`"BioCapPeerMdns"`) shared by both roles. Acquired
by whichever role starts first; released only once **neither** role is active (`releaseLockIfIdle()`).
This matters because the server registers *and* the client discovers from the same singleton — naive
release-on-stop would kill multicast for the other role.
...
### `PeerDevice` — [data/link/model/PeerDevice.kt](../app/src/main/java/com/biocap/app/data/link/model/PeerDevice.kt)

Plain data class representing a fully resolved peer:

```kotlin
data class PeerDevice(
    val name: String,  // mDNS service instance name, e.g. "BioCap-A-Pixel7"
    val host: String,  // resolved IPv4 address, e.g. "192.168.1.42"
    val port: Int      // port the WebSocket server is listening on
)
```

### `PeerNaming` — [data/link/PeerNaming.kt](../app/src/main/java/com/biocap/app/data/link/PeerNaming.kt)
...
```kotlin
PeerNaming.advertise("A", "Pixel7")          // → "BioCap-A-Pixel7"
PeerNaming.prefixOf("BioCap-A-Pixel7")       // → "A"
PeerNaming.matchesPrefix(name, localPrefix)  // client-side filter
```

`prefixOf` tolerates `NsdManager`'s collision-rename suffix (e.g. `BioCap-A-Pixel7 (1)`).
...
[`PeerLinkManager`](../app/src/main/java/com/biocap/app/data/link/PeerLinkManager.kt), which owns the
mDNS lifecycle alongside the WebSocket and applies the prefix filter. Injecting the mDNS service
straight into a second consumer would fight the manager over the shared listener and multicast lock.
...
`PeerNaming.matchesPrefix` before exposing it — so a UI never has to think about pair scoping.
...
- **Discovery is not free.** It holds the multicast lock and keeps the Wi-Fi chipset receiving
  multicast, so stop it once the user has picked a peer — `PeerLinkManagerImpl` does exactly this at
  connect time, and restarts it if the link drops.
...
- **`PeerMdnsService` is a singleton** shared by both roles. Calling `startDiscovery()` while already
  discovering is a no-op, and the multicast lock is only released when register *and* discover are
  both idle.
...
|---|---|
| Discovery silently does nothing | `discoverServices()` threw — check Logcat for `PeerMdnsService` / `NsdManager` errors |
