 ...
 * [com.biocap.app.data.prefs.SettingsRepository]) into the mDNS instance name so several
 * server/client pairs can share one Wi-Fi without crossing wires.
 *
 * A server advertises `BioCap-<prefix>-<model>`; a client only connects to peers whose embedded
 ...
 */
object PeerNaming {
    private const val BASE = "BioCap"

    /** Build the advertised instance name, e.g. `BioCap-A-Pixel7`. */
     ...
     * Tolerant of NsdManager's collision-rename suffix (e.g. `BioCap-A-Pixel7 (1)`).
     */
